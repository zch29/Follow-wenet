...
class MultiModeJoiner(torch.nn.Module):
    ...
    def _split_heads(self, x: torch.Tensor) -> torch.Tensor:
        ...
        *prefix, d = x.shape
        if d != self.join_dim:
            raise ValueError("invalid last dim: {}, expect {}".format(
                d, self.join_dim))
        return x.contiguous().view(*prefix, self.num_heads, self.head_dim)
    ...
    def _merge_heads(self, x: torch.Tensor) -> torch.Tensor:
        ...
        *prefix, h, dh = x.shape
        if h != self.num_heads or dh != self.head_dim:
            raise ValueError("invalid head dims: ({}, {}), expect ({}, {})".
                             format(h, dh, self.num_heads, self.head_dim))
        return x.contiguous().view(*prefix, h * dh)
    ...
    def _sigmoid_cross_attention_pruned(self, enc: torch.Tensor,
                                        pred: torch.Tensor) -> torch.Tensor:
        """
        HAT 模式的 pruned 版本（用于 k2 rnnt pruning）。

        输入：
        - enc:  [B, T, S, D]（已投影到 join_dim，且在 S 维与 pred 对齐）
        - pred: [B, T, S, D]（已投影到 join_dim）

        输出：
        - ctx: [B, T, S, D]

        说明：
        - 这里计算的是“逐对 (t,u)”的 sigmoid 门控，不做跨时间聚合，因此可在 pruned 网格上直接计算。
        """
        if enc.ndim != 4 or pred.ndim != 4:
            raise ValueError("pruned hat expects enc/pred as 4D tensors")
        if enc.shape[:3] != pred.shape[:3]:
            raise ValueError("pruned hat expects enc/pred to share [B,T,S]")
        # q/k/v: [B, T, S, H, Dh]
        q = self._split_heads(self.w_query(self.ln_q(pred)))
        kv_in = self.ln_kv(enc)
        k = self._split_heads(self.w_key(kv_in))
        v = self._split_heads(self.w_value(kv_in))
        # scores: [B, T, S, H]
        scores = (q * k).sum(-1) / math.sqrt(float(self.head_dim))
        alpha = torch.sigmoid(scores)
        # ctx: [B, T, S, H, Dh] -> [B, T, S, D]
        ctx = self._merge_heads(alpha.unsqueeze(-1) * v)
    ...
    def _compute_hat_log_probs_with_checkpoint(self, enc: torch.Tensor, pred: torch.Tensor) -> torch.Tensor:
        # 封装 _sigmoid_cross_attention 和 _hat_log_probs 为一个函数，便于 checkpoint
    ...
    def forward(self,
        ...
        if mode == "ctc":
            if enc.ndim != 3:
                raise ValueError("ctc mode only supports enc as [B,T,D]")
        ...
        if mode == "aed":
            if enc.ndim != 3 or pred.ndim != 3:
                raise ValueError("aed mode only supports enc/pred as 3D")
        ...
        # HAT 模式：启用 Gradient Checkpointing
        if enc.ndim == 4 and pred.ndim == 4:
            #[AI修改 开始位置 20260115 支持k2_pruning的pruned_HAT前向] 作用：支持 k2 rnnt pruning 的 pruned HAT 前向，避免构造 [B,T,U,*] 大张量
            ctx = self._sigmoid_cross_attention_pruned(enc, pred)
            h = torch.tanh(pred + ctx)
            return self._hat_log_probs(h)
            #[AI修改 结束位置 20260115 支持k2_pruning的pruned_HAT前向]
        #[AI修改 开始位置 20260115 HAT模式使用checkpoint降显存] 作用：使用 checkpoint 换取显存，解决 HAT 模式 OOM 问题
        if enc.requires_grad and pred.requires_grad:
             return torch.utils.checkpoint.checkpoint(
                self._compute_hat_log_probs_with_checkpoint,
                enc,
                pred,
                use_reentrant=False  # 推荐设为 False，避免某些反向传播问题
            )
        else:
            return self._compute_hat_log_probs_with_checkpoint(enc, pred)
        #[AI修改 结束位置 20260115 HAT模式使用checkpoint降显存]
