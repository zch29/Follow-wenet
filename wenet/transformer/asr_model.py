...
class ASRModel(torch.nn.Module):
    ...
    def decode(
        ...
        offline_aed_weight: float = 0.0,
        offline_aed_nbest: int = 0,
        offline_aed_max_len: int = 0,
        offline_aed_eos_id: int = -1,
        offline_aed_attend_window: int = 0,
        offline_aed_max_consecutive_repeats: int = 0,
        ...
        if isinstance(self, Transducer) and any(m in methods for m in [
            ...
            try:
                ...
                #[AI修改 开始位置 20260115 推理侧Trick1Trick2模式] 作用：Trick1+Trick2 推理侧模式（共享 predictor + ALSD）
                if 'rnnt_hat_aed_alsd_beam_search' in methods:
                    beam_results = []
                    for i in range(encoder_out.size(0)):
                        result = self.beam_search_hat_aed_alsd(
                            speech=speech[i:i + 1],
                            speech_lengths=speech_lengths[i:i + 1],
                            beam_size=beam_size,
                            decoding_chunk_size=decoding_chunk_size,
                        ...
                        tokens, score = result if isinstance(
                            result, (list, tuple)) else (result, 0.0)
                        beam_results.append(
                            DecodeResult(tokens=tokens, score=score))
                    results['rnnt_hat_aed_alsd_beam_search'] = beam_results
                #[AI修改 结束位置 20260115 推理侧Trick1Trick2模式]

                #[AI修改 开始位置 20260112 新增HAT流式离线AED重打分解码]
                # 作用：先用 HAT/RNNT（流式）生成 nbest，再用离线 AED（全序列 cross-attn）重打分重排序
                if 'rnnt_hat_offline_aed_rescoring' in methods:
                    rerank_results = []
                    aed_w = float(offline_aed_weight)
                    for i in range(encoder_out.size(0)):
                        hyps, hat_scores, enc_out = self.beam_search_nbest(
                            speech=speech[i:i + 1],
                            speech_lengths=speech_lengths[i:i + 1],
                            beam_size=beam_size,
                            decoding_chunk_size=decoding_chunk_size,
                            ...
                            ctc_weight=ctc_weight,
                            transducer_weight=self.transducer_weight,
                            nbest=offline_aed_nbest,
                        )
                        best_idx = 0
                        best_score = hat_scores[0] if len(hat_scores) > 0 else 0.0
                        if aed_w != 0.0:
                            for j, hyp in enumerate(hyps):
                                aed_lp = self.offline_aed_logprob(enc_out, hyp)
                                comb = float(hat_scores[j]) + aed_w * float(aed_lp)
                                if j == 0 or comb > best_score:
                                    best_score = comb
                                    best_idx = j
                        rerank_results.append(
                            DecodeResult(tokens=hyps[best_idx], score=best_score))
                    results['rnnt_hat_offline_aed_rescoring'] = rerank_results
                #[AI修改 结束位置 20260112 新增HAT流式离线AED重打分解码]

                #[AI修改 开始位置 20260112 新增离线AED自解码模式]
                # 作用：离线 AED 自身 greedy 生成（需要用 max_len/eos 等启发式停止）
                if 'rnnt_offline_aed_greedy_search' in methods:
                    aed_results = []
                    for i in range(encoder_out.size(0)):
                        tokens = self.offline_aed_greedy_search_from_encoder(
                            encoder_out=encoder_out[i:i + 1],
                            max_len=offline_aed_max_len,
                            eos_id=offline_aed_eos_id,
                            aed_attend_window=offline_aed_attend_window,
                            max_consecutive_repeats=offline_aed_max_consecutive_repeats,
                        )
                        aed_results.append(DecodeResult(tokens=tokens))
                    results['rnnt_offline_aed_greedy_search'] = aed_results
                #[AI修改 结束位置 20260112 新增离线AED自解码模式]
