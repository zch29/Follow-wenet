...
def main():
    ...
    from wenet.utils.train_utils import freeze_modules
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        #[AI修改 开始位置 20260206 训练过程统计已消费样本数] 作用：epoch保存时同步写入累计训练量，便于估算覆盖率
        if hasattr(executor, '_collect_seen_stats_global'):
            try:
                info_dict.update(executor._collect_seen_stats_global())
            except Exception:
                pass
