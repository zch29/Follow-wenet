...
#[NEW 开始位置 20260205 支持忽略shape不匹配的checkpoint加载] 作用：当 config/词表/模块结构变化时，跳过 shape 不一致的参数以便复用可匹配权重
def load_checkpoint_unmatched(model: torch.nn.Module, path: str) -> dict:
    rank = int(os.environ.get('RANK', 0))
    logging.info('[Rank {}] Checkpoint: loading (unmatched allowed) from {}'.
                 format(rank, path))
    checkpoint = torch.load(path, map_location='cpu', mmap=True)
    ...
    for k, v in model_state_dict.items():
        if k not in current:
            continue
        cur_v = current[k]
        if hasattr(v, 'shape') and hasattr(cur_v, 'shape') and v.shape != cur_v.shape:
            if rank == 0:
                skipped.append((k, tuple(v.shape), tuple(cur_v.shape)))
            continue
        filtered[k] = v
    ...
    if rank == 0:
        ...
        for (k, src_shape, dst_shape) in skipped:
            logging.info("skip mismatched tensor: {} {} -> {}".format(
                k, src_shape, dst_shape))
    ...
    if os.path.exists(info_path):
        with open(info_path, 'r') as fin:
            configs = yaml.load(fin, Loader=yaml.FullLoader)
    return configs
#[结束位置 20260205 支持忽略shape不匹配的checkpoint加载]
