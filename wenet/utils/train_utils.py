...
def add_model_args(parser):
    ...
    parser.add_argument('--checkpoint_unmatched',
                        action='store_true',
                        default=False,
                        help='Allow size mismatch when loading checkpoint')
...
def check_modify_and_save_config(args, configs, symbol_table):
    if args.train_engine in ["torch_ddp", "torch_fsdp"]:
        #[AI修改 开始位置 20260119 支持DDP/FSDP下bf16混合精度] 作用：允许 --use_amp --dtype bf16 走 bf16 autocast（无需GradScaler）
        if args.use_amp:
            if args.dtype == "bf16":
                configs["dtype"] = "bf16"
                args.dtype = "bf16"
        ...
        #[AI修改 结束位置 20260119 支持DDP/FSDP下bf16混合精度]
    elif args.train_engine == "deepspeed":
    ...
    if int(os.environ.get('RANK', 0)) == 0:
        #[AI修改 开始位置 2026-02-23 修改原因] 添加日志定位 Rank 0 是否卡死在保存配置
        logging.info("Rank 0: Saving config to {}...".format(args.model_dir))
        saved_config_path = os.path.join(args.model_dir, 'train.yaml')
        with open(saved_config_path, 'w') as fout:
            data = yaml.dump(configs)
            fout.write(data)
        logging.info("Rank 0: Config saved.")
        #[AI修改 结束位置 2026-02-23 修改原因] 添加日志定位 Rank 0 是否卡死在保存配置
...
def wrap_cuda_model(args, model, configs=None):
    ...
    if args.train_engine == "torch_ddp":  # native pytorch ddp
        ...
        model.to(device)
        #[AI修改 开始位置 2026-02-23 修改原因] DDP初始化前增加 barrier 防止连接重置，并捕获初始化异常
        try:
            logging.info("Rank {} waiting for barrier before DDP init...".format(os.environ.get('RANK', 0)))
            torch.distributed.barrier()
            logging.info("Rank {} passed barrier, starting DDP init...".format(os.environ.get('RANK', 0)))
            model = torch.nn.parallel.DistributedDataParallel(
                model, find_unused_parameters=not grad_ckpt)
            logging.info("Rank {} DDP init success.".format(os.environ.get('RANK', 0)))
        except RuntimeError as e:
            logging.error("Rank {} DDP init failed: {}".format(os.environ.get('RANK', 0), e))
            logging.error("Possible causes: Rank 0 died (OOM/Segfault), Network issues, or Firewall blocking.")
            raise e
        #[AI修改 结束位置 2026-02-23 修改原因] DDP初始化前增加 barrier 防止连接重置，并捕获初始化异常
...
def init_optimizer_and_scheduler(args, configs, model):
    ...
    #[AI修改 开始位置 20260205 冻结模块时仅优化可训练参数] 作用：使用 --freeze_modules 冻结参数后，避免优化器/ZeRO 为冻结参数分配状态
    if len(groups) > 0:
        filtered_groups = []
        for g in groups:
            trainable = [p for p in g['params'] if p.requires_grad]
            if len(trainable) > 0:
                filtered_groups.append({**g, 'params': trainable})
        params = filtered_groups
    else:
        params = [p for p in model.parameters() if p.requires_grad]
    if isinstance(params, list) and len(params) == 0:
        raise RuntimeError("未找到可训练参数：请检查 --freeze_modules 是否冻结了全部模块")
    #[AI修改 结束位置 20260205 冻结模块时仅优化可训练参数]
    ...
    if args.train_engine == "deepspeed":
        ...
        #[AI修改 开始位置 20260205 deepspeed仅接收可训练参数] 作用：冻结模块时避免 deepspeed 为冻结参数分配优化器/ZeRO状态
        ds_model_params = None
        if isinstance(params, list) and len(params) > 0 and isinstance(params[0], dict):
            ds_model_params = [p for g in params for p in g.get('params', [])]
        elif isinstance(params, list):
            ds_model_params = params
        ...
        model, optimizer, _, scheduler = deepspeed.initialize(
            ...
            model_parameters=ds_model_params if ds_model_params is not None else model.parameters())
        #[AI修改 结束位置 20260205 deepspeed仅接收可训练参数]
...
def init_scaler(args):
    scaler = None
    if args.use_amp:
        #[AI修改 开始位置 20260119 支持DDP下bf16混合精度] 作用：bf16不做loss scaling，避免无意义开销
        if args.dtype == "bf16":
            scaler = None
            ...
            logging.error("not supported device: {}".format(args.device))
        #[AI修改 结束位置 20260119 支持DDP下bf16混合精度]
    elif args.train_engine == 'torch_fsdp':
        ...
        with torch.no_grad():
            ...
            if info_dict["save_states"] == "model_only" and rank == 0:
                convert_zero_checkpoint_to_fp32_state_dict(model_dir,
                                                           save_model_path,
                                                           tag=tag)
                #[AI修改 开始位置 20260119 禁用rm以便可回退] 作用：将可删除目录改为重命名保留，避免直接删除
                try:
                    src = os.path.join(model_dir, str(tag))
                    if os.path.isdir(src):
                        dst = os.path.join(model_dir, "{}_可删除".format(tag))
                        if not os.path.exists(dst):
                            os.rename(src, dst)
                except Exception:
                    pass
                #[AI修改 结束位置 20260119 禁用rm以便可回退]
...
def batch_forward(model, batch, scaler, info_dict, device):
    ...
    autocast = {
        ...
        amp_autocast(enabled=dtype is not None,
                     ...
                     cache_enabled=False),
        "torch_ddp":
        #[AI修改 开始位置 20260119 支持DDP下bf16 autocast] 作用：scaler=None时仍可基于dtype启用autocast
        amp_autocast(enabled=dtype is not None, dtype=dtype)
        if dtype is not None else nullcontext(),
        #[AI修改 结束位置 20260119 支持DDP下bf16 autocast]
        "torch_fsdp":
...
def batch_backward(model, scaler, info_dict):
    ...
    #[AI修改 开始位置 20260119 支持DDP下bf16混合精度] 作用：bf16不需要GradScaler，避免误报断言
    if use_amp and info_dict.get("dtype", "fp32") == "fp16":
