...
#[AI修改 开始位置 20260115 引入MultiModeJoiner优化实现] 作用：导入优化后的 MultiModeJoiner，包含 Gradient Checkpointing
from wenet.transducer.joint_optimized import MultiModeJoiner as OptimizedMultiModeJoiner
#[AI修改 结束位置 20260115 引入MultiModeJoiner优化实现]
...
from wenet.transformer.zipformer2_encoder import Zipformer2Encoder
#[AI修改 开始位置 20260205 允许加载shape不匹配的checkpoint] 作用：支持 --checkpoint_unmatched 跳过 shape 不一致的参数
from wenet.utils.checkpoint import load_checkpoint, load_checkpoint_unmatched, load_trained_modules
#[AI修改 结束位置 20260205 允许加载shape不匹配的checkpoint]
...
WENET_ENCODER_CLASSES = {
    ...
    #[AI修改 开始位置 20260115 注册Zipformer2编码器] 作用：注册 Zipformer2 编码器适配类，便于最快跑通
    "zipformer2": Zipformer2Encoder,
    #[AI修改 结束位置 20260115 注册Zipformer2编码器]
}
...
WENET_JOINT_CLASSES = {
    ...
    # 多模式联合器：在同一 joiner 下切换 HAT/AED/CTC/LM 行为（论文 All-in-One ASR）
    # 使用方式：在 yaml 中设置 joint: "multi_mode_joiner"，并在 joint_conf 里配置维度与 head 数等
    # joint_conf 常用字段：
    # - enc_output_size: encoder 输出维度（投影前）
    # - pred_output_size: predictor 输出维度（投影前）
    # - join_dim: joiner 统一空间维度（需能被 num_heads 整除）
    # - num_heads / ff_hidden / dropout_rate / blank_id / eps: 可选
    "multi_mode_joiner": OptimizedMultiModeJoiner,
    #[AI修改 开始位置 20260115 兼容multi_mode_joint别名] 作用：添加 multi_mode_joint 别名，兼容配置文件中的不同写法
    "multi_mode_joint": OptimizedMultiModeJoiner,
    #[AI修改 结束位置 20260115 兼容multi_mode_joint别名]
}
...
def init_speech_model(args, configs):
    ...
    #[AI修改 开始位置 20260115 兼容多模Transducer无attention_decoder] 作用：调试打印 configs 信息，并增强 multi_mode_joiner 的判断逻辑
    print(f"DEBUG: model_type={model_type}, joint_type={configs.get('joint', 'unknown')}")
    if model_type == "transducer" and configs.get('model_conf',{}).get('rnnt', False):
        decoder = None
    elif model_type == "transducer" and configs.get('joint', 'transducer_joint') == 'multi_mode_joiner':
        decoder = None
    elif 'decoder_conf' not in configs:
        print("DEBUG: No decoder_conf found, skipping decoder initialization.")
        decoder = None
    else:
        decoder = WENET_DECODER_CLASSES[decoder_type](vocab_size, encoder.output_size(), **configs['decoder_conf'])
    #[AI修改 结束位置 20260115 兼容多模Transducer无attention_decoder]
...
def init_model(args, configs):
    ...
    if hasattr(args, 'checkpoint') and args.checkpoint is not None:
        if hasattr(args, 'checkpoint_unmatched') and args.checkpoint_unmatched:
            infos = load_checkpoint_unmatched(model, args.checkpoint)
        else:
