...
#train_config=conf/conformer_aed_conformer_0.2B.yaml
dir=exp/zipformer_debug
#dir=exp/conformer_debug
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  mkdir -p $dir
  num_gpus=$(echo $ASCEND_RT_VISIBLE_DEVICES | awk -F "," '{print NF}')
  # Use "hccl" for gpu if it works, otherwise use "gloo"
  # NOTE(xcsong): deepspeed fails with gloo, see
  #   https://github.com/microsoft/DeepSpeed/issues/2818
  dist_backend="nccl"

  # train.py rewrite $train_config to $dir/train.yaml with model igput
  ...
  torchrun --nnodes=$num_nodes --nproc_per_node=$num_gpus \
    ...
    wenet/bin/train.py \
      ...
      --config $train_config \
      --data_type  $data_type \
      --train_data data_debug/$train_set/dev/data.jsonl \
      --cv_data data_debug/$train_set/train/data.jsonl \
