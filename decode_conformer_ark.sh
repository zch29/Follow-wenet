...
decode_checkpoint=step_104999.pt
decode_yaml=step_104999.yaml
decode_nj=20
  ...
  for test_set in "${test_sets[@]}"; do
    ...
    $cpu_cmd -j "wenet-decode" -pj "none" JOB=1:$decode_nj $log_dir/decode.JOB.log --cmd \
	  ...
	  --hat_label_weight $hat_label_weight \
          ...
          --offline_aed_weight $offline_aed_weight \
          --offline_aed_nbest $offline_aed_nbest \
          --offline_aed_max_len $offline_aed_max_len \
          --offline_aed_eos_id $offline_aed_eos_id \
          --offline_aed_attend_window $offline_aed_attend_window \
          --offline_aed_max_consecutive_repeats $offline_aed_max_consecutive_repeats \
          --result_dir $result_dir/$test_set/log/JOB \
	        --num_decoding_left_chunks $num_decoding_left_chunks \
          ${decoding_chunk_size:+--decoding_chunk_size $decoding_chunk_size} \
	        ${simulate_streaming:+--simulate_streaming}" \
        2>&1 | tee $log_dir/log.JOB &
    #[结束位置 20260115 解码参数透传recognize]
        touch $log_dir/.done
  done
  wait
fi
