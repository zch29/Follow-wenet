...
class SentenceStats:
    def __init__(self):
        ...
        self.error_sentences = 0
    
    def add_sentence(self, has_error):
        self.total_sentences += 1
        if has_error:
            self.error_sentences += 1
    
    def get_error_rate(self):
        if self.total_sentences == 0:
            return 0.0
        return (self.error_sentences / self.total_sentences) * 100

class BadCaseCollector:
    def __init__(self):
        self.bad_cases = set()  # 使用set来自动去重
        
    def format_ref_text(self, tokens):
        """格式化参考文本，只在英文单词之间加空格
        Args:
            tokens: 分词后的列表
        Returns:
        ...
        def flush_eng():
            if current_eng:
                formatted_parts.append(' '.join(current_eng))
                current_eng.clear()
        
        for token in tokens:
            if is_english(token):
                current_eng.append(token)
            else:
                flush_eng()
                formatted_parts.append(token)
        
        flush_eng()  # 处理最后剩余的英文词
        return ''.join(formatted_parts)
    
    def add_case(self, ref):

        # 将错误样本格式化并添加到集合中
        self.bad_cases.add(self.format_ref_text(ref))
    
    def save_to_file(self, filename):
        if not filename:
            return
        with codecs.open(filename, 'w', 'utf-8') as fh:
            for case in sorted(self.bad_cases):  # 排序以保持输出稳定
                fh.write(case+'\n')
...
class Calculator:
    ...
    def calculate(self, lab, rec):
        ...
        while True:
            if self.space[i][j]['error'] == 'cor':  # correct
                ...
                if len(lab[i]) > 0:
                    ...
                    # 记录替换错误对
                    pair = (lab[i], rec[j])
                    if pair in self.sub_pairs:
                        self.sub_pairs[pair] += 1
                    else:
                        self.sub_pairs[pair] = 1
                result['lab'].insert(0, lab[i])
                result['rec'].insert(0, rec[j])
        ...
        has_error = result['sub'] > 0 or result['ins'] > 0 or result['del'] > 0
    ...
    # 添加获取替换错误对的方法
    def get_substitution_pairs(self):
        # 按出现次数降序排序
        sorted_pairs = sorted(self.sub_pairs.items(), key=lambda x: x[1], reverse=True)
        return sorted_pairs
...
if __name__ == '__main__':
    ...
    bad_case_collector = BadCaseCollector()  # 创建收集器实例
    cluster_file = ''
    ignore_words = set()
    tochar = False
    verbose = 1
    ...
    with codecs.open(hyp_file, 'r', 'utf-8') as fh:
        for line in fh:
            ...
            if not array: 
                continue
            fid = array[0]
            value = [''] if len(array) == 1 else (characterize(' '.join(array[1:])) if tochar else array[1:])
            
            rec_set[fid] = normalize(value, ignore_words, case_sensitive, split) if value else ''
    ...
    for line in open(ref_file, 'r', encoding='utf-8'):
        ...
        lab = normalize(value, ignore_words, case_sensitive, split)
        rec = rec_set[fid]   
        for word in rec + lab:
        ...
        sent_stats.add_sentence(result['has_error'])
        if verbose and result['has_error']:
            bad_case_collector.add_case(lab)
            print('\nutt: %s' % fid)
            if result['all'] != 0:
    ...
    # 在打印完总体 WER 后添加替换错误分析
    print('===========================================================================')
    print('Substitution Error Analysis:')
    sub_pairs = calculator.get_substitution_pairs()
    if sub_pairs:
        print('Reference -> Hypothesis: Count')
        for (ref, hyp), count in sub_pairs:
    ...
    if result['all'] != 0:
        ...
        wer = 0.0
    print('Overall WER -> %4.2f %%' % wer, end=' ')
    print('N=%d C=%d S=%d D=%d I=%d' %
          (result['all'], result['cor'], result['sub'], result['del'],
           result['ins']))
    
    print(f"Total Sentences: {sent_stats.total_sentences}")
    print(f"Error Sentences: {sent_stats.error_sentences}")
    print(f"Sentence Error Rate: {sent_stats.get_error_rate():.2f}%")
    if not verbose:
        print()
    ...
    if verbose:
        ...
        print('===========================================================================')
        if badcase_file:
            bad_case_collector.save_to_file(badcase_file)
